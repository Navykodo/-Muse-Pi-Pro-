...
int main(int argc, const char * argv[]) {
    ...
    // start scan...
    // Some A1 firmware/accessory-board combinations respond differently to
    // normal/force/typical scan commands. Try several modes and print the
    // exact result instead of silently blocking.
    op_result = drv->startScan(false, false);
    if (SL_IS_FAIL(op_result)) {
        fprintf(stderr, "startScan normal legacy failed: %x\n", op_result);
        op_result = drv->startScan(true, false);
    }
    if (SL_IS_FAIL(op_result)) {
        fprintf(stderr, "startScan force legacy failed: %x\n", op_result);
        op_result = drv->startScan(false, true);
    }
    if (SL_IS_FAIL(op_result)) {
        fprintf(stderr, "startScan typical failed: %x\n", op_result);
        goto on_finished;
    }
    printf("Scan started. Waiting for scan data...\n");
    ...
    while (1) {
        ...
        op_result = drv->grabScanDataHq(nodes, count, 2000);

        if (SL_IS_OK(op_result)) {
            printf("grabbed count=%zu\n", count);
            ...
            fprintf(stderr, "grabScanDataHq failed: %x count=%zu\n", op_result, count);
        }
